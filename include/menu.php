<div class="banner-bg">
	<video autoplay="autoplay" muted="muted" width="100%" loop >
		<source src="<?php echo $option['link'];?>/images/danangtime.mp4" type="video/mp4">
	</video>
	<div class="banner" style="width:100%; left:0 !important;">
		<div class="header">	 
			<div class="logo fadeInLeft" data-wow-delay="0.5s">
			<a href="<?php echo $option['link'];?>"><img src="<?php echo $option['link'];?>/images/logo.png" height="100" alt="blueriverdn.com"/></a>
			</div>	
			<div class="top-menu">
				<span class="menu"></span> 
				<ul class="cl-effect-1">
					<li><a class="scroll" href="<?php echo $option['link'];?>">TRANG CHỦ</a></li>
					<li><a class="scroll" href="<?php echo $option['link'];?>/controller/booking_room.php">ĐẶT PHÒNG</a></li>
					<li><a class="scroll" href="#footer-hotel">THÔNG TIN</a></li>
					<li><a class="scroll" href="#slide-hotel">HÌNH ẢNH</a></li>
					<li><a class="scroll" href="<?php echo $option['link'];?>/controller/contact.php">LIÊN HỆ</a></li>
				</ul>
			</div>
			<div class="clearfix"></div>	 
	 	</div>